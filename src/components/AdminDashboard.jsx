...
const AdminDashboard = () => {
    ...
    const handleWhatsAppConfirm = async (booking) => {
        ...
        } catch (error) { console.error("Error in WhatsApp confirm: ", error); }
    };
                            ...
                            <tbody className="divide-y divide-gray-700">
                                {bookings.map((booking) => (
                                    <tr key={booking.id} className="hover:bg-gray-750 transition-all">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${booking.status === 'confirmed' ? 'bg-green-500/10 text-green-500 border border-green-500/20' :
                                                    booking.status === 'completed' ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20' :
                                                        booking.status === 'no-show' ? 'bg-red-500/10 text-red-500 border border-red-500/20' :
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center justify-center gap-2">
                                                {/* WhatsApp / Confirm */}
                                                <button onClick={() => handleWhatsAppConfirm(booking)} title="Confirm & WhatsApp" className="bg-green-600 hover:bg-green-500 p-2 rounded-lg transition-all text-white">
                                                ...
                                                {/* Completed Button */}
                                                <button onClick={() => updateStatus(booking.id, "completed")} title="Mark as Completed" className="bg-blue-600 hover:bg-blue-500 p-2 rounded-lg transition-all text-white">
                                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>
                                                </button>

                                                {/* No Show Button */}
                                                <button onClick={() => updateStatus(booking.id, "no-show")} title="Mark as No-Show" className="bg-orange-600 hover:bg-orange-500 p-2 rounded-lg transition-all text-white">
                                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                                </button>

                                                {/* Delete Button */}
                                                <button onClick={() => handleDelete(booking.id)} title="Delete Booking" className="bg-red-600/20 hover:bg-red-600 text-red-500 hover:text-white p-2 rounded-lg transition-all">
