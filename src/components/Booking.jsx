...
function BookingApp() {
    ...
    const generateCalendarDays = () => {
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            if (dateObj < new Date(today.setHours(0, 0, 0, 0)) || adminHolidays.includes(dateStr)) {
                                ...
                                <React.Fragment key={s.id}>
                                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
                                        <div style={{
                                            ...
                                            boxShadow: step === s.id ? '0 0 10px #fff' : 'none'
                                        }}>
                                            {step > s.id ? '✓' : s.id}
                                        </div>
                                        <span style={{ fontSize: '10px', color: '#fff', marginTop: '5px', textTransform: 'uppercase', opacity: step >= s.id ? 1 : 0.5 }}>{s.label}</span>
                                    </div>
                                    {idx < arr.length - 1 && (
